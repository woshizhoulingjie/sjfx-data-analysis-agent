#!/usr/bin/env python3
"""Run a cooperative 200 MiB SJFX end-to-end and per-feature workload profile."""
...
SCHEMA_VERSION = "sjfx-all-feature-profile/1.0"
MIB = 1024 ** 2
...
def build_subtopic_tree(documents):
    ...
    return {"kind": "group", "name": "200MB NVD CVE 数据集", "children": topics}
...
def build_folder_context(documents, dataset_bytes):
    ...
    return {
        ...
        "coverage": {
            ...
            "limitations": ["目录摘要使用 200MB 数据集中的真实 CVE 代表记录。"],
        },
        "documents": [
...
def projection(census, all_requests, context_window_tokens):
    ...
    return {
        ...
        "warning": "这是依据真实调用吞吐得到的容量推算，不是把全部 200MB 正文送入共享 GPU 的实测耗时。",
    }


def render_report(path, result, census, model_summary, feature_summary, projection_data):
    model_by_stage = {row["stage_name"]: row for row in model_summary}
    lines = [
        "# SJFX 约 200MB 真实数据全功能画像测试",
        "",
        "## 测试口径",
        "",
    ...
    lines.extend([
        ...
        "## 模型调用指标",
        "",
        "`Context P95 = P95(input_tokens + output_tokens)`；`上下文占用 P95 = P95(context_tokens / 65536)`。",
        "",
        "| 功能阶段 | 调用数 | 输入 Token 合计 | 输出 Token 合计 | Context P95 | 占用 P95 | TTFT P95(ms) | E2E P95(ms) |",
        "|---|---:|---:|---:|---:|---:|---:|---:|",
    ])
    for stage_name in sorted(model_by_stage):
        item = model_by_stage[stage_name]
        requests = [row for row in result["all_model_requests"] if row.get("stage_name") == stage_name and row.get("status") == "ok"]
        lines.append("| {} | {} | {:,} | {:,} | {:.0f} | {:.3f}% | {:.1f} | {:.1f} |".format(
            stage_name,
            item.get("model_call_count") or 0,
            sum(int(row.get("input_tokens") or 0) for row in requests),
            sum(int(row.get("output_tokens") or 0) for row in requests),
            item.get("context_tokens_p95") or 0,
            item.get("context_occupancy_pct_p95") or 0,
            item.get("ttft_ms_p95") or 0,
            item.get("client_e2e_ms_p95") or 0,
        ))
    lines.extend([
        "",
        "## 功能级耗时",
    ...
    lines.extend([
        ...
        "- 以上是容量推算，不冒充 200MB 全量模型推理实测。实际模型用例使用同一数据集中的非重叠真实 CVE 原文。",
...
def main(argv=None):
    ...
    try:
        ...
        result.update({
            ...
            "full_200mb_projection": projection_data,
        })
        ...
        if projection_data:
            atomic_write_json(output_dir / "full_200mb_projection.json", projection_data)
        atomic_write_json(output_dir / "all_features_summary.json", result)
        if census and projection_data:
            render_report(
                output_dir / "200MB真实数据全功能测试报告.md",
                result,
                census,
                all_model_summary,
                feature_summary,
