...
class FrontendTaskContractTests(unittest.TestCase):
    ...
    def test_large_trees_and_summaries_use_bounded_lazy_endpoints(self):
        ...
        self.assertIn("SJFX_API_TOKEN_KEY", script)
        self.assertIn("window.sessionStorage.getItem(SJFX_API_TOKEN_KEY)", script)
        self.assertIn("window.sessionStorage.removeItem(SJFX_API_TOKEN_KEY)", script)
        self.assertIn("SJFXAuth.request", script)
        ...
        self.assertIn("deep_file_summary", script)
        self.assertIn("preliminary_file_summary", script)
        self.assertIn("type=${encodeURIComponent(summaryType)}", script)
        self.assertIn("window.SJFXAuth", script)
        self.assertIn("SJFXAuth.request", script)
        self.assertIn("ensureToken({ force: true })", shell)
    ...
    def test_model_probe_executes_a_bounded_generation(self):
        app_source = (PROJECT_ROOT / "app.py").read_text(encoding="utf-8")
        self.assertIn("Reply with exactly SJFX_OK", app_source)
        self.assertIn('"generation_tested": True', app_source)
        self.assertIn("generation_reply", app_source)
