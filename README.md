...
当前版本已经完成四项核心加固：大资料包按 **最多 500 个逻辑文件一批持续处理并断点续跑**；中文主题目录采用稳定节点身份和主归属校验；证据链升级为逐结论核验；推荐研究方向改为基于正式分析树和可回查证据的十维评分。模型只负责受控的命名、摘要和语言增强，不替代本地解析、建树、检索与证据校验。
...
- 大包轻量预览预算在 Worker 切片之间累计，任务让出或重启后从持久化游标继续；默认将所有有效逻辑文件放入受控后台深析队列，并支持通过配置关闭后台回填，采用“有界批次 + 用户问题/选择提升优先级”。
...
- 大资料包的轻量预览按有界 Worker 切片遍历清点范围，深析默认最多以 500 个逻辑文件为一个持久化批次；重型文件会自动缩小实际批次；每个完成文件立即保存检查点，后续按代表性、用户问题或人工选择继续晋升。
...
大资料包不会只清点首批文件。系统先对全库完成盘点、有界基础解析和可检索证据入口，再把所有有效逻辑文件放入可恢复的深度处理队列。每批最多 500 个逻辑文件，同时受计算工作量上限保护；默认连续处理到全量完成。用户可以随时“结束本次运行”，已完成的单文件检查点永久保留，之后可直接续跑，或通过关系召回、关键词、自然语言要求和手动选择重新排列剩余队列。优先级只改变顺序，不会丢弃普通待处理文件。
...
- 已清点文件按有界切片建立轻量预览和可检索状态；有效逻辑文件进入深析队列，默认每批最多 500 个；问题命中文件和人工选择只调整优先级；
- 完整解析结果逐文件写入 sidecar，包级内存只保留有限内容画像；
- 大包默认跳过高相似度和 embedding 语义聚类，但仍根据全部已解析文件的正文头、中、尾和章节生成内容主题树；
- 可通过 `LARGE_PACKAGE_BATCH_FILES` 调整逻辑批量大小，默认 500、代码强制范围为 1–500；扫描 PDF、超大表格和长音视频会按预计工作量自动缩小实际批次。后台仍拆成受控小任务，模型并发和 CPU 解析并发保持独立安全上限。
...
LARGE_PACKAGE_INITIAL_PARSE_FILES=500
LARGE_PACKAGE_DEEPEN_BATCH_FILES=500
LARGE_PACKAGE_BATCH_FILES=500
LARGE_PACKAGE_BACKGROUND_BATCH_FILES=500
...
LOGICAL_PARTITION_BYTES=1048576
LOGICAL_PARTITION_ROWS=10000
MAX_LOGICAL_UNITS_PER_CONTAINER=200000
```
...
前端“同构文件关联”是独立工作模块，适用于同一批结构相似的信件、通知、批复、工单等文件。基础解析完成后进入该模块，点击“开始关联分析”。

系统会：

- 识别文件编号、日期、发件方、收件方、主题、事项编号、期限和签发人等常用字段；
- 自动发现模板中重复出现的其他“标签：值”公共字段；
- 为每份文件生成确定性的简短含义说明和字段原文依据；
- 优先根据明确编号、正文引用、事项编号、参与方和时间识别回复、跟进、引用及同一事项关系；
- 将可靠关系组织成事项线程和时间线；
- 标记字段缺失、引用文件缺失和可能未回复等待核对项；
- 用分页台账处理数千份文件，不进行全量两两比较。

若系统没有发现至少两个覆盖大多数文件的公共字段，会明确显示“结构不足”，不会强行生成关系。该分析只读取已经持久化的解析结果，不会重复执行 OCR，也不会改变原始目录、主题目录或原文件。

主要接口：

```text
POST /api/homogeneous-analysis/<scan_id>   启动后台关联分析
GET  /api/homogeneous-analysis/<scan_id>   分页读取台账、关系、事项和异常
GET  /api/homogeneous-record/<scan_id>     读取单文件字段证据和上下游关系
```
